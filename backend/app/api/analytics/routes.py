...
def get_dashboard_summary():
    ...
    Returns: occupancy, active alerts, avg response time, active UIDs
    """
    try:
        with analytics_db.session_scope() as session:
            # Count active journeys (current occupancy)
            active_uids = session.query(PersonJourney).filter(
                PersonJourney.is_active == True
            ).count()
            
            # Count high-severity behaviors (active alerts) in last hour
            one_hour_ago = datetime.utcnow() - timedelta(hours=1)
            active_alerts = session.query(BehaviorEvent).filter(
        ...
        return jsonify({
            'success': True,
            'occupancy': active_uids,
            'activeAlerts': active_alerts,
            'avgResponseTime': 3.2,  # TODO: Calculate from alert system
...
def get_dashboard_anomalies():
    ...
    Returns: Recent behavior events with detection info
    """
    try:
        with analytics_db.session_scope() as session:
...
@analytics_bp.route('/dashboard/occupancy', methods=['GET'])
def get_dashboard_occupancy():
    """
    Get occupancy data by geo-fence
    Returns: Current occupancy vs capacity per fence
    """
    try:
        with analytics_db.session_scope() as session:
            # Get active detections in each fence (last 60 seconds)
            time_threshold = datetime.utcnow() - timedelta(seconds=60)
            
            # Count unique people (by persistent_id) in each fence
            fence_counts = session.query(
                PersonDetection.fence_name,
                func.count(func.distinct(PersonDetection.persistent_id)).label('count')
            ).filter(
                PersonDetection.fence_name.isnot(None),
            ...
            # Process actual counts
            for fence_name, count in fence_counts:
                if fence_name:
                    # Capitalize first letter for display
                    display_name = fence_name.capitalize()
                    ...
                    # Calculate trend (compare to 2 minutes ago)
                    prev_threshold = datetime.utcnow() - timedelta(minutes=2)
                    prev_count = session.query(
                        func.count(func.distinct(PersonDetection.persistent_id))
                    ).filter(
                        func.lower(PersonDetection.fence_name) == fence_name.lower(),
                        PersonDetection.in_geo_fence == True,
                        PersonDetection.timestamp >= prev_threshold,
                        PersonDetection.timestamp < time_threshold
                    ).scalar() or 0
                    
                    if count > prev_count:
                        occupancy[display_name]['trend'] = 'increasing'
                    elif count < prev_count:
                        occupancy[display_name]['trend'] = 'decreasing'
        ...
        logger.error(f"Error getting occupancy: {str(e)}")
        return jsonify({
            'success': False,
            'error': str(e)
        }), 500
...
def get_dashboard_system_health():
    ...
    Returns: FPS, accuracy, uptime, active UIDs, etc.
    """
    try:
        with analytics_db.session_scope() as session:
            # Get active UIDs
            ...
            # Calculate ID switch rate (journeys per hour)
            one_hour_ago = datetime.utcnow() - timedelta(hours=1)
            recent_journeys = session.query(PersonJourney).filter(
                PersonJourney.created_at > one_hour_ago
            ).count()
        
        # TODO: Get real metrics from stream_manager
        health = {
            'fps': 28.5,
            'detectionAccuracy': 94.2,
            'anomalyPrecision': 87.6,
            'uptime': 99.8,
            'idSwitchRate': round(recent_journeys / max(active_uids, 1), 1),
            'activeUIDs': active_uids
        }
