...
class AnalyticsDatabase:
    ...
    def save_behavior(self, detection_id, behavior_type, severity, confidence,
        ...
        Args:
            ...
            behavior_type: Type of behavior (loitering, running, suspicious, crowd)
    ...
    # UPDATE THESE METHODS in app/api/analytics/database.py

    def update_hourly_stats(self, camera_id, hour_timestamp):
        """
        Calculate and update hourly statistics for a camera
        UPDATED: Supports new behavior types (violence, fallen, fire, smoke)
        ...
        with self.session_scope() as session:
            ...
            # Count behaviors (UPDATED for new types)
    ...
    def generate_daily_report(self, camera_id, date):
        ...
        with self.session_scope() as session:
            ...
            # Get all behaviors for the day (UPDATED)
            behaviors = session.query(BehaviorEvent).join(PersonDetection).filter(
                PersonDetection.camera_id == camera_id,
    ...
    def generate_daily_report(self, camera_id, date):
        ...
        with self.session_scope() as session:
            ...
            report.total_loitering = sum(1 for b in behaviors if b.behavior_type == 'loitering')
            report.total_running = sum(1 for b in behaviors if b.behavior_type == 'running')
            report.total_suspicious = sum(1 for b in behaviors if b.behavior_type == 'suspicious')
            report.total_crowd = sum(1 for b in behaviors if b.behavior_type == 'crowd')
